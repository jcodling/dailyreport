...
if sudo pmset repeat wake MTWRFSU 02:55:00 2>/dev/null; then
  WAKE_STATUS="Scheduled persistent daily wake at 2:55 AM (survives reboots)"
else
  WAKE_STATUS="WARNING: Could not set repeating wake — run manually: sudo pmset repeat wake MTWRFSU 02:55:00"
fi
